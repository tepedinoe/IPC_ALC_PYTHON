...
# Each value in the marriages dictionary is a list. These are the indexes of the elements in those lists.
HUSBAND_KEY_INDEX = 0
WIFE_KEY_INDEX = 1
...
def print_death_age(people_dict):
    ...
    for name in people_dict:
        value = people_dict[name]
        full_name = value[NAME_INDEX]
        birth_year = value[BIRTH_YEAR_INDEX]
        death_year = value[DEATH_YEAR_INDEX]

        print(full_name, death_year-birth_year)
...
def count_genders(people_dict):
    ...
    for gender in people_dict:
        value = people_dict[gender]
        both_genders = value[GENDER_INDEX]
    ...
    print('Genders:')
    print(f'Numbers of females: {female_total}')       
    print(f'Numbers of males: {male_total}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    """
    print('Marriages:')
    for marriage in marriages_dict:
        value = marriages_dict[marriage]
        husband_name = value[HUSBAND_KEY_INDEX]
        wife_name = value[WIFE_KEY_INDEX]
        wedding_year = value[WEDDING_YEAR_INDEX]
        male_list = people_dict[husband_name]
        female_list = people_dict[wife_name]
        husband_age = wedding_year - male_list[BIRTH_YEAR_INDEX]
        wife_age = wedding_year - female_list[BIRTH_YEAR_INDEX]
        print(f'{male_list[0]} (Age:{husband_age}) > {wedding_year} < {female_list[0]} (Age: {wife_age})')
